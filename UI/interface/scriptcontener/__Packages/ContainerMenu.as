class ContainerMenu extends ItemMenu
{
   ...
   function updateFavoriteText()
   {
      if(!this.isViewingContainer())
      {
         this.BottomBar_mc.SetButtonText(!this.ItemCard_mc.itemInfo.favorite ? "$Favorite" : "$Unfavorite",3);
      }
      else
      {
         this.BottomBar_mc.SetButtonText("",3);
   ...
   function onQuantityMenuSelect(event)
   {
      ...
      else if(this.InventoryLists_mc.ItemsList.selectedEntry.enabled)
      {
   ...
   function StartItemTransfer()
   {
      ...
      else if(this.InventoryLists_mc.ItemsList.selectedEntry.count <= InventoryDefines.QUANTITY_MENU_COUNT_LIMIT)
      {
