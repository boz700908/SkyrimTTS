...
- **Enchanted weapon charge readout** — the remaining charge of an enchanted weapon is announced in the inventory, container, barter and gift menus as `charge X out of Y` (e.g. `charge 240 out of 1500`). Integers tell you exactly how many casts you have left, while a percentage like "3%" would be ambiguous (could be one cast left, could be twelve). Works for both runtime-enchanted and base-form enchanted weapons. Not announced on armor, rings or amulets (the engine does not drain charge from those).
- **Enchantment slider at the enchanting table is accessible** — when crafting a new enchantment at an altar, after picking the weapon, the effect and the soul gem, the engine shows a slider that balances charge count against effect duration/magnitude. Sighted players see this as a horizontal bar; blind players had no idea it existed. The plugin now announces "Charges slider, uses X, cost Y" when the slider opens and re-announces the new values each time you move it left or right. You can finally find the sweet spot between "few powerful uses" and "many weak uses" yourself.
- **Soul gem selection on weapon recharge (T key) is accessible** — pressing T on an enchanted weapon in the inventory opens a sub-list of usable soul gems. The plugin now detects this sub-mode, announces "Select soul gem", and reads each gem as you scroll through them. Once you confirm (or cancel), the recharged weapon is re-announced normally.
...
### Bug fixes
- **Quest objectives in the scanner no longer leak raw `Alias` tags** — many quest objectives reference NPCs, locations or items via tags like `<Alias=Target>`, `<Alias.ShortName=QuestGiver>` or `<alias=waifu>` (lowercase variant used by some mods). The plugin used to only resolve the strict uppercase `<Alias=...>` form, so the other two leaked through and the scanner announced things like `Apportez cet objet à alias short name questgiver`. All three formats are now recognised and resolved to the actual NPC name (e.g. `Apportez cet objet à Lydia`).
- **Stealth status no longer announced while standing** — the plugin used to read the sneak meter's text field on every HUD tick and announce `Hidden` / `Detected` / `Caution` whenever it changed. Problem: the engine leaves that text field filled even after the player straightens up and the icon fades out, so the plugin would announce stealth states while the player was clearly walking around in plain sight (and sighted players saw nothing on screen). The plugin now checks the `_alpha` of the stealth meter container before announcing — if the icon is faded out, no announcement. If the alpha can't be read (modded HUD with a different layout), the old behaviour is kept as a safety net.
- **Enemy distance now consistent between X / Shift+X / auto-aim and the scanner** — the lock-on key (X), the auto-aim and the scanner used to report different distances for the same enemy. The lock-on measured to the actor's feet (`GetPosition()`) while the scanner measured to the centre of the 3D bounding box. For a giant or a flying dragon this could differ by 100-200 units, which was confusing ("the scanner says 850 units away, X says 720"). All three now use the same reference point (centre of the bounding box, what the scanner already does), so the announced distance matches across features.
