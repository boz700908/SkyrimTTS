...
### New features
- **Perk tree (constellation) accessibility** — the 3D perk constellation that opens when you select a skill in the Stats Menu was barely usable for blind players: only the raw perk name was spoken, with no clue about what the perk did, what was required, where it sat in the branch, or how to navigate it. The plugin now automatically announces the centered perk with everything you need: name, current and total rank, full description (e.g. "Half magicka cost for novice restoration spells"), required skill level if any, and the reason it cannot be learned yet (rank already maxed, level too low, missing prerequisite, etc.). The information is read directly from the game data (BGSPerk), so the language follows the game and there is no waiting on the SWF cards to update
- **Perk-tree navigation hotkeys (1, 2, 3, 4)** — while inside the constellation, four hotkeys give a structural view that simply could not be obtained otherwise:
  - **Press 1** — announces the perks this one leads to (its children in the tree). Example: `This perk leads to 6 perks: Regain, Twin healing, Apprentice restoration, Regeneration, Respite, Ward absorb`. Lets you decide whether following this branch is worth it before you spend the point.
  - **Press 2** — announces the prerequisites of the current perk (its parents). Example: `This perk requires 1 prerequisite: Novice restoration`. Or `Root perk, no prerequisite` if it sits at the start of the branch.
  - **Press 3** — lists every perk in the current skill tree that you can buy *right now* (level high enough, prerequisites owned). Example: `3 perks can be learned: Regain, Apprentice restoration, Regeneration`. Skim the whole tree without navigating star by star.
  - **Press 4** — lists every perk you already own in the current skill tree, with the rank you have on each multi-rank perk. Example: `You have 5 perks in this tree: Novice restoration, Regain rank 1 of 2, Twin healing, Regeneration, Respite`. Useful to remember your build and decide where to invest next.
...
### Internal
- New `src/menu_perks.h` builds an in-memory index of every perk in every skill tree at plugin startup (~180 perks across 18 skills). The index keeps the BGSPerk pointer, the skill tree node, the actor value of the parent skill, and the lowercase name for display lookups. All perk-tree announcements (centered perk info, hotkeys 1-4) read from this index for instant native lookups instead of waiting on the SWF
- Stats Menu detection of the centered perk in the constellation: filtered by scale > 110 to ignore perks that briefly pass through during ring rotations, with a `|x|` minimum tie-breaker when several perks share the same scale, plus a 150 ms debounce so a perk announced once is not re-announced if the user briefly passes back over it
